...
router
  ...
  .post(redirectLogin, function (req, res) {
    ...
    form.parse(req, async function (err, fields, files) {
      ...
      fs.writeFileSync(`${form.uploadDir}/${id}/body.html`, html, (error) => {
        console.log(error);
      });
      ...
      article.moreSections =
        fields.moreSections == "" ? [] : fields.moreSections.split("|");
...
let changePathWithRegex = (body, id) => {
  ...
  array.forEach((elem) => {
    ...
    body = body.replace(
      elem[1],
      `<img alt='photo'src='/upload/${id}/${name}.webp'>`
    );
  });
